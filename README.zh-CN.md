...
- 提供中文本地化的 React/Vite 作者工作台，可显示真实 API 项目树、空 workspace 引导、章节/场景元数据编辑、场景草稿、协作草稿箱、Context Pack 检查、连续性 QA、工作流事件查看、图/时间线预览、待审事实处理、本地 txt/md/docx 文件或文件夹导入、agent settings 管理和更新检查。
...
当前已验证的 Windows 构建中，与 updater 相关的本地产物是 NSIS setup 可执行文件及其 Tauri updater 签名 `StoryGraph Agent_0.1.6_x64-setup.exe.sig`。除非构建输出实际改变，不要再写 `nsis.zip` updater artifact。Tauri updater 签名只用于程序内更新校验，和 Windows Authenticode 代码签名不同；后端 sidecar 与安装器的生产级 Authenticode 签名仍是单独发布步骤。
