...
- Ensure workbench demo data, sampleData, empty-workspace placeholders, and graph/timeline preview fixtures are labeled as UI/sample state unless backed by graph or draft store provenance.
...
- Treat imported local documents as reader/library input until a backend workflow explicitly turns them into Draft Store drafts, StyleSample Store samples, or pending candidates with provenance.
...
- Do not treat React `sampleData`, desktop UI state, empty-workspace onboarding UI, or author-visible labels as canon unless retrieved from Graph Store with provenance.
...
- Do not treat imported txt/md/docx reader content as canon or hard context unless it has a Draft Store, StyleSample Store, or reviewed graph provenance ref.
- Do not build a Context Pack for a placeholder project/scene shown only by UI sample data.
