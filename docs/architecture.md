...
- Source Store 负责**项目资料与来源追踪**：持久保存来源元数据和抽取文本，只有作者显式选择后才能进入 Agent 输入；它不是 canon 或 Draft Store，初版也不保存原文件字节。
...
- Source Document 是本地私有来源资料，不是 canon、Draft、Proposal、CandidateFact、Style Sample、Event Log 或 workflow checkpoint。
...
- Source Store 导入本身只写 Source Document；它不得顺带写 Proposal Store、Draft Store、StyleSample Store、Candidate Store、Graph Store 或 Event Log。后续每个转换动作仍需独立、显式、permission-gated 的后端 API。Agent 从来源正文生成的章节/场景结构必须先表现为作者可编辑的 `project_structure_draft`，只有作者接受并显式应用后才可创建 Chapter/Scene 节点。LLM 从资料抽出的设定必须先表现为作者可编辑的 `fact_draft`，并以真实 Draft Store 来源满足 CandidateFact provenance 后才可提交 pending review，不得自动提交 canon。Agent 对话和局部改写只能写 Proposal Store；即使成功把选中段落替换为完整 `scene_draft` proposal，也必须由作者在协作草稿箱中显式接受/提升后才会进入 Draft Store。
...
日常写作以目录、正文和 Agent 三栏组织。章节控制展开与收起，场景是打开文稿的叶节点；在其他页面点击当前场景也应返回正文。Agent 与检查/设置共用右侧面板，不能挤出第四栏；窄屏按顺序呈现，避免覆盖正文。章/场景元数据改名仍通过显式保存写入原有带来源的 API。

目录改名和高级属性编辑固定 API、项目及节点 ID，不能把当前场景作为旧表单的保存目标。标题只提交 title，高级表单只提交实际改动字段；同类标题/高级编辑互斥。跨作用域切换和关闭页面保护未保存的正文、提案与元数据；点击已经打开的场景只切回正文，不触发丢弃编辑。

Agent 打开、选区和续写入口复用父工作区唯一的正文与提案状态，不复制草稿缓存。发送清单展示目标场景、输出语言和确切已保存 Draft；完整输入、来源和联网选项分级收起。未保存正文与语言阻断保持可见，独立保存后再发送，防止异步保存闭包引用旧 Draft。结果只进入 Proposal Store，仍经差异查看、接受和转草稿，不能直接覆盖正文或正典。当前单次请求不隐含多轮聊天记忆。

结构语言校验补充短标题并在首次应用前检查实际提案内容。旧英文结构不因界面语言或软件更新被改写，已完整应用的提案保持可验证幂等重试。专名和稳定标识不自动翻译；没有自动付费重试。

最小场景尚未填写的规划字段按 `context_pack_v1` 投影为空字符串，并报告 `missing_context`；不能把数据库里的 null 直接传给必填字符串而导致 500。讨论和基于已保存 Draft 的续写可使用带缺失提示的上下文。完整场景生成继续遵守原有必要信息门槛；投影默认值不写回图谱。

## 26. 更新准备与文件释放

v0.1.15 的原生更新准备在受管后端退出后，对 Windows sharing/lock violation 32/33 最多等待 10 秒。其他错误立即报告，持续占用仍阻止安装；不能为了通过检查扩大进程终止范围。准备工作运行于阻塞工作线程，窗口继续响应。错误阶段（下载/验证、准备、安装器启动）与后端恢复结果分别展示；恢复失败时不能提示继续反复安装。

0.1.13/0.1.14 中已有的一次性检查可能阻止接收本次修复。此时须保存编辑、从托盘退出，再将最新签名安装器覆盖安装到原目录一次；无需卸载或删除小说工作区。真实 one-file 双进程与暂时/持续锁验收见 `docs/windows-update-recovery.md`。

## 27. 目录语言修复

图谱节点与关系类型通过独立中英文UI catalog 显示可读名称；稳定类型标识不改变，作者的目录标题与摘要不作为界面字串自动翻译。对于旧版生成后保存的错误语言内容，作者可以显式生成目录语言修复提案。该操作仅发送章/场景已有的白名单文本字段，不读取或发送正文、来源资料、关系或人物设定。

修复复用 canon_patch 的窄 structured_json schema，冻结项目输出语言、稳定节点 ID 与原字段值。生成和接受均不写正典；只有作者显式应用已接受且版本匹配的修订，才按全量归属、类型、字段、语言与原值检查更新原节点。图谱、目录和时间线共用这些节点，不能通过重建目录产生新 ID 或重复关联。JSON/内存后端使用锁内克隆、原子持久化与状态交换；无法保证批量原子性的后端必须明确拒绝。完成事件作为重复应用证据，不能仅凭当前值等于新值认定已应用。

## 28. 正文预览、编辑与新篇章生成

作者工作区以真实 Draft 正文为阅读来源。点击章节时按场景顺序拼接已保存正文；点击场景时默认预览，并提供显式编辑模式。阅读与编辑共用同一份父级草稿状态，不能通过切换模式丢失未保存文字。没有 Draft 的结构节点必须明确说明“仅有规划、尚无正文”，给出手动写作、资料选段载入和 Agent 起草入口；不能把摘要伪装成正文，也不能猜测旧导入引用对应的原文范围。

正文选区记录确切已保存 Draft ID、原样选中文字与 UTF-16 起止位置。后端校验边界、Unicode 字符完整性及原文相等后才生成改写；重复出现的相同段落依靠位置区分。预览中的标题、按钮和跨场景选择不作为正文范围。Agent 修改继续生成独立 Proposal，按其版本记录的精确 Draft 基线显示删除/新增；颜色配合符号与标签，长文比较有明确计算上限和退化说明。接受及采用仍是独立动作。

资料载入正文是作者显式选择原文范围的本地操作，校验 Source ID、版本/校验和、精确范围和当前 Draft ID，保留来源摘要信息，不改动 Graph Store。原样载入只允许资料语言与项目正文语言一致；跨语言资料需明确选择为 Agent 参考，让输出继续遵守项目语言，不能把英文原稿标记为中文。
